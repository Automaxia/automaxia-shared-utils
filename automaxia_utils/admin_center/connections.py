"""Resolver de conexoes de banco de dados — broker da plataforma.

Cada produto que precisa de credencial de banco de dados nao guarda mais
host/porta/usuario/senha em variavel de ambiente local. Chama a plataforma
(`GET /api/database-connection/resolve`) que devolve as credenciais
decriptadas com TTL de 5 minutos.
...
Fluxo:
    ...
    with admin.get_db_session('cliente_prod') as session:
        rows = session.execute(text('select * from public.itens')).fetchall()
...
Cache:
    ...
    min). Quando a plataforma retorna `version` diferente do cacheado,
...
# ============================================================
# DTO — espelha ResolvedConnection do plataforma-backend
# ============================================================
@dataclass
class ResolvedConnection:
    """Snapshot decriptado de uma conexao de banco.

    Identico ao `models.ResolvedConnection` do plataforma-backend. Mantem
...
class ConnectionResolver:
    ...
    def get_engine(self, alias: str, **engine_kwargs: Any) -> Any:
        """Cria `sqlalchemy.create_engine(...)` com pool curto."""
        try:
    ...
    def _fetch(
        ...
        """Chama `GET /api/database-connection/resolve` na plataforma."""
